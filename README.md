...
# Com conta-isco (honeytoken) - qualquer atividade gera CRITICAL
.\IR-O365.ps1 -HoneytokenUsers @("canary@dominio.com")
```
...
| `-HoneytokenUsers` | string[] | `@()` | Contas-isco (canary) - qualquer atividade (sucesso ou falha) gera finding CRITICAL |
...
|---|---|
| 5.9.0 | `New-HTMLReport`: novo painel "Sumário Executivo" (painel inicial por defeito do relatório) com score de risco 0-100 (`crit*25 + high*10 + med*3 + low*1`, capado em 100) e classificação CRITICO/ALTO/MEDIO/BAIXO/OK; narrativa em linguagem simples gerada a partir do tenant, período analisado, contagens por severidade e das entidades com maior concentração de findings correlacionados (`$Script:AttackTimeline`, agora persistido em scope de script pelo módulo 26 `Build-AttackTimeline` e não apenas exportado para CSV); lista de ações prioritárias (até 6) derivada de um mapa estático técnica MITRE → remediação, cruzado com as técnicas presentes em findings CRITICAL/HIGH e ordenado por frequência; gráfico de distribuição de findings por tática MITRE, calculado em PowerShell e renderizado como barras CSS (sem JS/SVG/bibliotecas externas, mantendo o relatório como ficheiro HTML único e portátil); tabela de top 5 entidades em risco reutilizando o Attack Timeline; folha de estilos `@media print` (variáveis de cor claras, oculta sidebar/topbar/filtros, força todos os painéis a `display:block`) e botão "Exportar / Imprimir" na topbar para gerar PDF/impressão a partir do browser |
| 5.8.0 | Módulo 05 `Get-ExchangeSuspiciousActivity`: novo check de protocolos legacy ativos por mailbox (`Get-CASMailbox` - IMAP/POP/ActiveSync, T1556.006, HIGH - contornam Legacy Auth block a nivel de tenant/CA) + deteção via UAL de alterações `Set-CASMailbox` (MEDIUM), exportado para `04_legacy_protocols_per_mailbox.csv` e `04_cas_mailbox_changes_ual.csv`; novo check de remoção de Litigation Hold via UAL `Set-Mailbox` (`LitigationHoldEnabled` -> False, T1070, HIGH - anti-forense), exportado para `04_litigation_hold_removed.csv`; módulo 06 `Get-SuspiciousOAuthApps`: novo check de credenciais de longa duração em apps/SPs (secrets/certs com validade > 2 anos, T1098.001, MEDIUM - persistência que sobrevive a rotações normais de credenciais), exportado para `05_apps_longlived_credentials.csv`; nova entrada na tabela `$auditQueries` do módulo 07 `Get-CriticalAuditEvents` (UAL): `DLP_Policy_Changes` (`New/Set/Remove/Disable/Enable-DlpCompliancePolicy`, T1562, HIGH - desativação de DLP pode preceder exfiltração) |
